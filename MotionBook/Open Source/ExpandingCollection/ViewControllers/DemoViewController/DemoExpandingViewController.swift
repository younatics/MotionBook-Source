...
extension DemoExpandingViewController {
    ...
    func addGestureToView(_ toView: UIView) {
        let gesutereUp = Init(UISwipeGestureRecognizer(target: self, action: #selector(DemoExpandingViewController.swipeHandler(_:)))) {
            $0.direction = .up
        }
        
        let gesutereDown = Init(UISwipeGestureRecognizer(target: self, action: #selector(DemoExpandingViewController.swipeHandler(_:)))) {
            $0.direction = .down
        }
