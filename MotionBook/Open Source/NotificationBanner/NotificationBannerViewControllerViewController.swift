...
class NotificationBannerViewController: UIViewController {
    ...
    private var exampleView: NotificationBannerView!
    
    override func viewDidLoad() {
        super.viewDidLoad()
        
        edgesForExtendedLayout = UIRectEdge()
        exampleView = NotificationBannerView(delegate: self)
        view = exampleView
...
extension NotificationBannerViewController: ExampleViewDelegate {
    ...
    internal func blockColor(at indexPath: IndexPath) -> UIColor {
        ...
        if indexPath == IndexPath(row: numberOfCells(for: indexPath.section) - 2, section: 0) {
            return MotionBookBannerColors().color(for: .warning)
        }
