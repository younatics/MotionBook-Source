...
open class YNSearchView: UIView, YNSearchMainViewDelegate, YNSearchListViewDelegate {
    ...
    open func ynSearchMainViewSearchHistoryChanged() {
        if let last = self.ynSearchMainView.ynSearchHistoryViews.last {
            if last.frame.origin.y + last.frame.height + 50 > self.frame.height {
                let size = CGSize(width: self.frame.width, height: last.frame.origin.y + last.frame.height + 50)
                ynScrollView.contentSize = size
                ynSearchMainView.frame = CGRect(origin: CGPoint(x: 0, y: 0), size: size)
            } else {
